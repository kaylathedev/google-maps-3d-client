  ...
  */
  ...
  class JSBeautifier {
    ...
    public function __construct(BeautifierOptions $options = null) {
      if (!$options)
        $this->options = new BeautifierOptions();
      $this->blank_state();
    }
    ...
    public function beautify($string, BeautifierOptions $options = null) {
      ...
      if (!in_array($this->options->brace_style, [static::BS_COLLAPSE, static::BS_EXPAND, static::BS_END_EXPAND])) {
    ...
    private function get_next_token() {
      ...
      if (strpos($this->wordchar, $c) !== false) {
        ...
        if (($this->parser_pos != strlen($this->input)) &&
          ...
          return [$c, static::TK_WORD];
        }

        // in is an operator, need to hack
        if ($c == 'in') {
          return [$c, static::TK_OPERATOR];
        }
      ...
      if (strpos('([', $c) !== false) return [$c, static::TK_START_EXPR];
      if (strpos(')]', $c) !== false) return [$c, static::TK_END_EXPR];

      if ($c == '{') return [$c, static::TK_START_BLOCK];
      if ($c == '}') return [$c, static::TK_END_BLOCK];
      if ($c == ';') return [$c, static::TK_SEMICOLON];
      if ($c == '/') {
        $comment = '';
        ...
        if ($this->input[ $this->parser_pos ] == '/') {
          ...
          return [$comment, static::TK_COMMENT];
        }
      }

      if (($c == "'") || ($c == '"') ||
        ($c == '/' && (($this->last_type == static::TK_WORD && in_array($this->last_text, ['return', 'do'])) ||
            (in_array($this->last_type, [
          ...
          if ($sep == '/') {
            ...
            while ($esc || $in_char_class || $this->input[ $this->parser_pos ] != $sep) {
              ...
              if ($this->parser_pos >= strlen($this->input)) {
                ...
                return [$resulting_string, static::TK_STRING];
              }
            }
          } else {
            // handle string
      ...
      if (in_array($c, $this->punct)) {
        ...
        return ($c == '=') ? [$c, static::TK_EQUALS] : [$c, static::TK_OPERATOR];
      }

      return [$c, static::TK_UNKNOWN];
    }
    ...
    private function handle_start_expr($token_text) {
      ...
      if ($this->last_text == ';' || $this->last_type == static::TK_START_BLOCK) {
        $this->append_newline();
      } elseif (in_array($this->last_type, [static::TK_END_EXPR, static::TK_START_EXPR, static::TK_END_BLOCK]) || $this->last_text == '.') {
        # do nothing on (( and )( and ][ and ]( and .(
      } elseif (!in_array($this->last_type, [static::TK_WORD, static::TK_OPERATOR])) {
        $this->append(' ');
    ...
    private function handle_start_block($token_text) {
      ...
      if ($this->options->brace_style == static::BS_EXPAND) {
        ...
        if (!in_array($this->last_type, [static::TK_OPERATOR, static::TK_START_EXPR])) {
    ...
    private function handle_word($token_text) {
      ...
      if ($this->last_type == static::TK_END_BLOCK) {
        if (!in_array($token_text, ['else', 'catch', 'finally'])) {
          ...
          if (in_array($this->options->brace_style, [static::BS_EXPAND, static::BS_END_EXPAND])) {
            $prefix = 'NEWLINE';
          } else {
            $prefix = 'SPACE';
            $this->append(' ');
      ...
      } elseif ($this->last_type == static::TK_SEMICOLON && in_array($this->flags->mode, ['BLOCK', 'DO_BLOCK'])) {
        $prefix = 'NEWLINE';
      ...
      if (in_array($token_text, ['else', 'catch', 'finally'])) {
        ...
        if ($token_text == 'function' && ($this->last_type == static::TK_START_EXPR || in_array($this->last_text, ['=', ',']))) {
    ...
    private function handle_string($token_text) {
      if (in_array($this->last_type, [static::TK_START_BLOCK, static::TK_END_BLOCK, static::TK_SEMICOLON])) {
        $this->append_newline();
      } elseif ($this->last_type == static::TK_WORD) {
        $this->append(' ');
      }
    ...
    private function handle_operator($token_text) {
      ...
      elseif (in_array($token_text, ['--', '++', '!']) ||
        (in_array($token_text, ['+', '-'])
          && in_array($this->last_type, [static::TK_START_BLOCK, static::TK_START_EXPR, static::TK_EQUALS, static::TK_OPERATOR])) ||
